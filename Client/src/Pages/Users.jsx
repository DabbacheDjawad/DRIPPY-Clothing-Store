...
const Users = () => {
    ...
    async function fetchUsers() {
      try {
        ...
        const response = await axios.get(
          "https://drippy-clothing-store.onrender.com/api/v1/users",
          {
        ...
        setResponse(err.response?.data?.message || "Failed to fetch users");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  async function handleRemoveUser(id) {
    try {
      await axios.delete(
        `https://drippy-clothing-store.onrender.com/api/v1/users/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  async function handleAdminSwitch(id) {
    try {
      const response = await axios.patch(
        `https://drippy-clothing-store.onrender.com/api/v1/users/switch/${id}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
